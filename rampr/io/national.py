...
def _read_use_make(
    ...
    """
    use_sheets = pd.read_excel(Path(use_xlsx), sheet_name=None, skiprows=[0,1,2,3])
...
def read_use_make():
    ...
    U = (
        use.iloc[:, 2:].apply(pd.to_numeric, errors="coerce").fillna(0.0)
    )
    ...
    V = (
        make.iloc[:, 2:].apply(pd.to_numeric, errors="coerce").fillna(0.0)
    )
    g = U.values[-1,:402]
    q = U.values[:402, -1]
